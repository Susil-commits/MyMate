    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-green-50 via-white to-teal-50">
      ...
      <div className="relative w-full max-w-md px-4 z-10 animate-fade-up">
        ...
        <div className="backdrop-blur-xl bg-white/70 rounded-3xl shadow-xl shadow-black/5 border border-white/50 p-8">
          ...
          <p className="text-gray-500 mb-8">Register and start earning.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select name="gender" value={form.gender} onChange={handleChange} required className="w-full px-4 py-3 bg-white/80 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition-all duration-200">
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            Already registered?{" "}
            <Link to="/driver/login" className="text-green-600 hover:text-green-700 font-semibold transition-colors">Sign In</Link>
          </p>
        </div>
      </div>
    </div>
...
function Field({ label, name, type = "text", value, onChange, required, placeholder, minLength }) {
  return (
    <div>
      ...
      <input type={type} name={name} value={value} onChange={onChange} required={required} minLength={minLength} placeholder={placeholder} className="w-full px-4 py-3 bg-white/80 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 outline-none transition-all duration-200" />
    </div>
  );
}